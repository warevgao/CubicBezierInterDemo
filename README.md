...
* **Performance Metrics**: Provides execution time statistics (average duration) in benchmark mode.  
  *Note: Execution time results are hardware-dependent and may vary across different devices. Timing measurements are intended for relative comparison on the same platform and should not be directly compared between systems with different hardware configurations.*
